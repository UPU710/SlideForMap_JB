...
slidevariablenames <- c("latitude", "longitude", "mmacht", "anrissflaeche", "rutschneigung")  # REL File Hangmuren_Daten_Final.csv
...
resolution <- 5            # the desired resolution for the total, if 0, no resampling   # in meter(?)
TWI_multiplier <- 1.236         # a multipication of the TWI
ls_density <- 0.05                       # the density of the randomly generated landslides; landslides/m^2
...
y_max <-  extent(dem)[4]  
y_min <- extent(dem)[3]   
ext <- extent(x_min, x_max, y_min, y_max)   # create the total extent of the area; --> array mit 4 elementen
...
TWI[TWI < 0] <- 0  # setze alle Minuswerte auf 0 
Slope[Slope < 0] <- 0   # setze alle Minuswerte auf 0 
TWI <- TWI * TWI_multiplier # add a multipication to the TWI

if (noNANs) { # creates a square of NoNan's research area.  # If noNANS == TRUE --> Run ohne NANs
  TWI[is.na(TWI)] <- 0    # set all NA to 0. glaube es h??tte gar keine NAs gehabt..(?)
  dem[is.na(dem)] <- 0    # set all NA to 0. glaube es h??tte gar keine NAs gehabt..(?)
  Slope[is.na(Slope)] <- 0    # set all NA to 0. glaube es h??tte gar keine NAs gehabt..(?)
}
...
if (slides_subsetting) { # create a subset over the study area  # slides_subsetting == TRUE.
...
tab_tot <- tab_tot[slidevariablenames]  # Choose only the 5 necessary columns
colnames(tab_tot) <- c("Latitude", "Longitude", "Maechtigkeit", "Flaeche", "Neigung.Rutschflaeche") # rename those columns

tab_tot <- tab_tot[!is.na(tab_tot[,2]),]  # choose all values of column 2 that are not NA. F??r y-Kolonne nicht n??tig, wenn bereits f??r x-Kolonne getan.
tab_tot <- tab_tot[!is.na(tab_tot[,3]),]  # dito. Removes the whole line when it contains a NA.
tab_tot <- tab_tot[!is.na(tab_tot[,4]),]  # dito
tab_tot <- tab_tot[!is.na(tab_tot[,5]),]
tab_tot <- data.frame(tab_tot) # make sure only the inputs with real numbers are accepted and format back to data frame (That solved a problem that Feiko had..).

data_area <- as.character(tab_tot$Flaeche) # make a character vector
data_area <- as.numeric(gsub("'", "", data_area))  # replace ' mit nichts und mache wieder numeric vector (In Excel-Daten werden 1000 manchmal dargestellt als 1'000, welche R nicht interpretieren kann).
# data_area[is.na(data_area)] <- 10   # set NA auf 10. (Falls es Probleme macht..).
tab_tot$Flaeche <- data_area # L??s Feiko: Ersetzen der Daten. Wenn R "immer noch NA drin hat", dann kann man es so beheben.
breaks <- seq(from = 0, to = max(data_area) + 10, by = 10) # create sequence from 0 to Max+10, by 10.
hist_data <- hist(data_area, breaks = breaks, plot = TRUE)  # default: FALSE
freq_data <- hist_data$counts/sum(hist_data$counts) # calculate density. I.e. frequency of the ls-amounts for each bin (e.g.: 2/33 = 0.0606)

#### 07: GENERATION OF RANDOM DISTRIBUTED LANDSLIDE COORDINATES ####
if (progress_display) {
  print("07: GENERATION OF RANDOM DISTRIBUTED LANDSLIDE COORDINATES")
}
n <- ceiling((x_max - x_min)*(y_max - y_min)*ls_density)	# Aufrunden auf ganze Anzahl ls. Number of total generated landslides, Anzahl hypothetischer Rutschungen (16000 Rutschungen/km2 = 0.016R/m2)
...
A_L <- seq(0.00001, max(breaks)*0.000001, 0.00001)  # Einheit ist km2. #classes of landslide areas are in 10 m^2 == 0.00001 km^2
loops <- 10000
rho_gamma_rand <-	runif(loops, 1.4, 1.7)			#1.6                        # Estimation of paramter: 10000 random-Werte zw 1.4 und 1.7
a_rand <- runif(loops, 0.5*10^(-4), 1.5*10^(-4)) 	#9.99*10^(-5)           # Estimation of paramter: 10000 random-Werte zw
s_rand <-  runif(loops, 1.9*10^(-10), 1.9*10^(-7))    # 5.26163e-08       # Estimation of paramter: 10000 random-Werte zw
SSE <- s_rand # create SSE vector of same length as vector s_rand. Werte werden dann ??berschrieben.
...
while (index < loops + 1)
{
  ...
  # probability distribution Malamut et al. 2004.
  freq_model <- p_A_L/sum(p_A_L) # Anteile of all bins between 0 and 1
  errors <- freq_model - freq_data
  SSE[index] <- sum(errors^2)   # alle 10'000 Werte von SSE ??berschrieben..(?)
  index <- index + 1
  # c # plot the 10'000 histograms.
}

# See distribution of the 3 parameters:
plot(rho_gamma_rand, SSE)
plot(a_rand, SSE)
plot(s_rand, SSE)



# Definition of the Parameters: rho_gamma, a, s:
rho_gamma <-	rho_gamma_rand[which(SSE == min(SSE[!is.na(SSE)]))]  #1.5	#1.4   # unklar...
a <-   a_rand[which(SSE == min(SSE[!is.na(SSE)]))]          #1.28*10^(-3)
s <-   s_rand[which(SSE == min(SSE[!is.na(SSE)]))]          #0.09*10^(-4)  #1.82*10^(-4)
p_A_L <- (1/(a*gamma(rho_gamma)))*((a/(A_L - s))^(rho_gamma + 1))*exp(-a/(A_L - s))  # probability distribution Malamut et al., 2004, (3). Mit Parametern rho_gamma, s, a. 
hist(p_A_L, breaks = seq(from = 0, to = 40000, by = 100)) 
# Landslide area is in square kilometers.

# Print histogram with every ls of the inventory as green point and red line for the calculated 3 parameter inverse gamma distribution:
# Code zur Zeit falsch..:
plot(breaks[2:length(breaks)], hist_data$counts / sum(hist_data$counts), pch = 19, col = "green4", lines(1000000*A_L, p_A_L/sum(p_A_L), col = "red", lwd = 3)
     # grid()
...
c<- 2       									# Quotient zwischen Breite 1 und L??nge 2
ls_width<- sqrt((ls_area*4)/(pi*c))				# Rutschbreite
ls_length<-ls_width*c   						# Rutschl??nge = Rutschbreite*2
...
soil_cohesion_unsaturated<- rnorm(n,mean_soil_cohesion,sd_soil_cohesion)              	# Bodenkoh??sion [kPa] SC:5.3(11.6),5.8(6.7)
...
if(random_soil_depth){ # create a random vector for all the soil depths, dependent on the slope
  soil_depth<-rnorm(n,mean_soil_depth,sd_soil_depth) * (1-pnorm(slope,1.35 *mean(data_xslope),0.75*sd(data_xslope)))		# Feldaufnahme mean(data_slope), sd(data_slope) Rutschk??rperm??chtigkeit/-tiefe [m], pnorm serve ad evitare che ci siano frane in zone ripide!
...
ls_volume<- ls_area*soil_depth*cos(slope_rad)					# Rutschvolumen = Rutschfl??che*-Tiefe
...
if(vegetation_included){
  ...
  if(variable_rootlat){
    ...
    root_lateral<- root_reinf_ls/1000       					# Laterale Wurzelverst??rkung [kN/m], 2-15
  }
